...
from src.schedulers.constraint_fixing.constraint_fixing_scheduler import (
    ConstraintFixingScheduler,
)
from src.schedulers.constraint_fixing.constraint_fixing_solver import (
    ConstraintFixingSolver,
)
...
def main():
    ...
    parser.add_argument(
        "--import_path", required=True, type=str, help="read json input from this path"
    )
    parser.add_argument(
        "--export_path",
        required=False,
        type=str,
        help="export json output to this path",
    )
    parser.add_argument(
        "-c",
        required=False,
    ...
    if (args.c is not None) + args.m + args.b + (args.hb is not None) + (
        args.g is not None
    ) != 1:
        handle_error(
...
def run_constraint_checker(
    ...
    [sports, events, general_constraints, data] = parse_input_constraint_checker(
        input_json
    )
...
def run_solver(
    ...
    use_backtracking_solver: bool,
    use_heuristic_backtracking_solver: int,
