...
def flatten_events_by_sport_to_list(
    input_dict: dict[str, dict[str, Event]]
) -> list[Event]:
    events = []
    for sport in input_dict:
...
def flatten_events_by_sport_to_dict(
    input_dict: dict[str, dict[str, Event]]
) -> dict[str, Event]:
    events = {}
    for sport in input_dict:
...
def flatten_events_by_sport_to_dict_with_scores(
    input_dict: tuple[float, dict[str, dict[str, Event]]]
) -> dict[str, Event]:
    events = {}
...
def widen_events_to_events_by_sport(
...
def add_global_variables(sports: dict[str, Sport], data, general_constraints):
    ...
    for sport_name in sports:
        ...
        for constraint in sport.constraints["required"]:
            global_variables.constraint_params[sport.name]["required"][
                constraint
            ] = sport.constraints["required"][constraint]
        for constraint in sport.constraints["optional"]:
            global_variables.constraint_params[sport.name]["optional"][
                constraint
            ] = sport.constraints["optional"][constraint]
    global_variables.constraint_params["general"] = general_constraints


def heuristic(
    assignments: dict[str, dict[str, Event]], optional_constraints, csp_instance
) -> float:
    if len(optional_constraints) == 0:
        return 1
    normalising_factor = sum(
        optional_constraint_heuristic.get_params()["weight"]
        for optional_constraint_heuristic in optional_constraints
    )
    ...
    for heuristic in optional_constraints:
        if heuristic.get_sport() is None:
            ...
            score += (
                take_average_of_heuristics_across_all_sports(
                    csp_instance, assignments, heuristic
                )
...
def calculate_fitness(
    assignments: dict[str, dict[str, Event]],
    constraints,
    optional_constraints,
    csp_instance,
    accept_invalid_solutions=False,
) -> float:
    ...
    for constraint in constraints:
        if constraint.get_sport() is None:
            constraints_broken += (
                len(constraint_check(constraint, assignments_flatten)) > 0
            )
        else:
            sport_name = constraint.get_sport().name
            constraints_broken += (
                len(constraint_check(constraint, assignments[sport_name])) > 0
            )
