class Sport:
    ...
    def __init__(self, name, possible_venues, num_teams, teams_per_game, match_duration, is_knockout=True,
        ...
        if group_stage is not None:
            self.has_group_stage = True
            self.group_size = group_stage['group_size']
            self.round_robin = group_stage['round_robin']  # Will always be true for now to decrease complexity of task
    ...
    def __str__(self) -> str:
        return f"""{{
            ...
            teams per game: {self.teams_per_game},
