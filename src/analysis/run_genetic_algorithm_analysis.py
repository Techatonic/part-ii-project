...
from src.schedulers.csop.genetic_algorithm.genetic_algorithm_scheduler import (
...
def compare_iterations():
    ...
    ax1 = ax.twinx()
    line_2 = ax1.plot(
        df["iterations"], df["runtimes"], label="Runtime (s)", color="orange"
    )
...
def compare_population_size():
    ...
    for size in avg_eval_score_by_population_size:
        avg_eval_score_by_population_size[size] = sum(
            avg_eval_score_by_population_size[size]
        ) / len(avg_eval_score_by_population_size[size])
...
def compare_iterations_and_population_size():
    ...
    eval_scores = [
        [[] for j in range(len(ga_iterations))] for i in range(len(populations))
    ]
    runtimes = [
        [[] for j in range(len(ga_iterations))] for i in range(len(populations))
    ]
    ...
    inverse_cmap = LinearSegmentedColormap.from_list(
        "gr", ["darkgreen", "green", "palegreen", "lightcoral", "red", "darkred"], N=256
    )
    ...
    draw_heatmap(
        ...
        ga_iterations,
        populations,
        "Heatmap of Population size and GA Iterations vs Performance",
    ...
    draw_heatmap(
        ...
        ga_iterations,
        populations,
        "Heatmap of Population size and GA Iterations vs Runtime",
        "Genetic Algorithm Iterations",
        "Population Size",
        None,
        None,
    ...
    draw_heatmap(
        avg_performance_per_second,
        cmap,
        ga_iterations,
        populations,
        "Heatmap of Population and GA Iterations vs Performance per Second",
        "Genetic Algorithm Iterations",
        "Population Size",
        None,
        None,
        "Evaluation Score / Second",
    )
