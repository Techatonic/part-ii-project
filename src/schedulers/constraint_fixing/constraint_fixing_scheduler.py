...
class ConstraintFixingScheduler(Scheduler, ABC):
    ...
    def __generate_constraint_checker_csp_problem(self, sport_name: str):
        ...
        csp_problem.data["num_total_events"] = self.sports_data[sport_name][
            "num_total_events"
        ]
        ...
        events_to_change = [
            self.events[sport_name][event_name]
            for event_name in events_to_change
            if event_name in self.events[sport_name]
        ]
        ...
        for event_to_change in events_to_change:
            ...
            if "team_time_between_matches" in sport.constraints["required"]:
                specific_min_start_day = self.sports_data[sport_name][
                    "min_start_day"
                ] + sport.constraints["required"]["team_time_between_matches"][
                    "min_time_between_matches"
                ] * (
                    self.sports_data[sport_name]["round_order"][0].round_index
                    - event_to_change.round.round_index
                )
                specific_max_finish_day = (
                    self.sports_data[sport_name]["max_finish_day"]
                    - sport.constraints["required"]["team_time_between_matches"][
                        "min_time_between_matches"
                    ]
                    * event_to_change.round.round_index
                ...
                specific_min_start_day = (
                    self.sports_data[sport_name]["min_start_day"]
                    + self.sports_data[sport_name]["round_order"][0].round_index
                    - event_to_change.round.round_index
                )
                specific_max_finish_day = (
            ...
            if (
                ...
                and len(self.sports_data[sport_name]["round_order"]) > 1
            ):
                handle_error(
                    ...
                    for day in day_order:
                        options.append(
                            Event(
                                ...
                                event_to_change.id,
                                venue=venue,
                                event_round=event_to_change.round,
                                day=day,
                                start_time=time,
