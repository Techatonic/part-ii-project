...
class GeneticAlgorithmScheduler(Scheduler, ABC):
    def __init__(
        self,
        solver: Type[GeneticAlgorithmSolver],
    ...
    def schedule_events(self) -> CompleteGames | None:
        ...
        for sport in self.sports:
            ...
            csp_data[sport.name] = {
                ...
                "comparator": lambda curr, other: curr.domain[0].round.round_index
        ...
        multisport_csp = self.solver(
            csp_data, self.forward_check, initial_population=self.initial_population
        )
        ...
        for sport_name in self.sports:
            ...
            for event_round in round_order:
                ...
                for match in matches:
                    ...
                    if "team_time_between_matches" in sport.constraints["required"]:
                        specific_min_start_day = min_start_day + sport.constraints[
                            "required"
                        ]["team_time_between_matches"]["min_time_between_matches"] * (
                            round_order[0].round_index - event_round.round_index
                        )
                        specific_max_finish_day = (
                            max_finish_day
                            - sport.constraints["required"][
                                "team_time_between_matches"
                            ]["min_time_between_matches"]
                            * event_round.round_index
                        )
                    else:
                        ...
                        specific_max_finish_day = (
                            max_finish_day - event_round.round_index
                        )
                    if (
                    ...
                    day_order = list(
                        range(specific_min_start_day, specific_max_finish_day + 1)
                    )
            ...
            csp_data[sport.name][
                "num_total_events"
            ] = match_num  # match_num at end of iteration = total number of matches

            for sport_specific_constraint in sport.constraints["required"]:
                multisport_csp.add_constraint(
            ...
            for optional_constraint in sport.constraints["optional"]:
                multisport_csp.add_optional_constraint(
                    optional_constraint,
        ...
        csp_data["num_total_events"] = sum(
            csp_data[sport_name]["num_total_events"]
            for sport_name in csp_data["sports"]
        )

        for required_constraint in self.data["general_constraints"]["required"]:
            multisport_csp.add_constraint(
                required_constraint,
                params=self.data["general_constraints"]["required"][
                    required_constraint
                ],
            )
        ...
        try:
            ...
            (
                ...
                eval_score,
                eval_by_iteration,
                time_taken_by_iteration,
            ) = multisport_csp.solve()
            end_time = time.time()
            if result is None:
                return None
