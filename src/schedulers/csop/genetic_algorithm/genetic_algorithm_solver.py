...
def generate_population(sports, data, num_iterations):
    ...
    for i in range(num_iterations):
        ...
        populations.append(
            csp_scheduler.schedule_events().complete_games["events"])
    return populations
...
class GeneticAlgorithmSolver(Solver):
    ...
    def solve(self, attempt=0):
        ...
        for iteration in range(max_iterations):
            fitness_of_population = [self.__calculate_fitness(
                assignments) for assignments in population]
            ...
            self.data["time_taken"][iteration + 1] = time.time() - \
            ...
            evaluation_by_iteration[iteration +
                                    1] = self.__calculate_fitness(fittest_assignments[0])
