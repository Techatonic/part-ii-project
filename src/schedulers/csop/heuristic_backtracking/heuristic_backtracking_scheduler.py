    ...
    def schedule_events(self) -> CompleteGames | None:
            ...
            csp_data.update(
                {
                    ...
                    or curr.domain[0].round.round_index
                    == other.domain[0].round.round_index
                ...
                while True:
                    ...
                    try:
                        ...
                        fitness = calculate_fitness(
                            widen_events_to_events_by_sport(result),
                            csp_problem.constraints,
                            csp_problem.optional_constraints,
                            csp_problem,
                        )
                        if result is None:
                            return None
        ...
        while len(total_events) > 1:
            ...
            for sport_key in [sport_1, sport_2]:
                sorted_options_by_optimality = sorted(
                    total_events[sport_key], key=lambda option: option[0], reverse=True
                )
            ...
            for individual_result in range(len(result)):
                ...
                eval_score = calculate_fitness(
                    events,
                    multisport_csp.constraints,
                    multisport_csp.optional_constraints,
                    multisport_csp,
                )
        ...
        eval_score = calculate_fitness(
            result,
            most_recent_csp.constraints,
            most_recent_csp.optional_constraints,
            most_recent_csp,
        )
